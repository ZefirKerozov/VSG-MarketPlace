...
const loadProducts = async () => {
    try {
        ...
        dataToJSON.filter(x => x.quantityForSale > 0).forEach(x => {
            ...
            buyBtn.addEventListener('click', showPopup);// Open and close pop-up functionality
            ...
            function showPopup(e) {
                ...
                const selectedQuantity = e.target.parentElement.querySelector('#quantity').value;
                const totalPrice = e.target.parentElement.parentElement.querySelector('.item-price').textContent.split(' ')[0] * selectedQuantity;
                divPopUp.innerHTML = `<p class="inside-pop-up">Are you sure you want to buy ${selectedQuantity} ${selectedQuantity > 1 ? 'items' : 'item'} for ${totalPrice} BGN?</p>
