...
<div class="item-card">
    ...
    <div class="item-info">
        <div class="price-category">
            <span class="item-price">300 BGN</span>
...
class MarketplaceItem extends HTMLElement {
    constructor() {
        super();
    ...
    _setUpProductData() {
        ...
        if (this.hasAttribute('price')) {
            this.shadowRoot.querySelector('.item-price').textContent =
                `${this.getAttribute('price')} BGN`;
        }
        if (this.hasAttribute('category')) {
            this.shadowRoot.querySelector('.item-category').textContent =
                this.getAttribute('category');
        }
        if (this.hasAttribute('quantity')) {
            const selectForm = this.shadowRoot.querySelector('#quantity');
            const quantity = this.getAttribute('quantity');
            for (let i = 1; i <= quantity; i++) {
                selectForm.innerHTML += `<option value="${i}">${i}</option>`;
            }
        }

        this.shadowRoot
            .querySelector('img')
            .addEventListener('click', onItemImageClick);

        this.shadowRoot
            .querySelector('.buy-btn')
            .addEventListener('click', showPopup);

        async function onItemImageClick(e) {
            const id = e.target.getRootNode().host.id;
            const itemData = await makeRequest({ path: `/Products/${id}` });
...
customElements.define('marketplace-item', MarketplaceItem);
